...
using LogEvent_2_sap = eviso15118::session_2_sap::logging::Event;
using LogEvent_2 = eviso15118::session_2::logging::Event;
...
std::ostream& operator<<(std::ostream& os, const eviso15118::session_2_sap::logging::ExiMessageDirection& direction) {
    using Direction = eviso15118::session_2_sap::logging::ExiMessageDirection;
    switch (direction) {
...
public:
    ...
    void operator()(const eviso15118::session_2_sap::logging::SimpleEvent& event) {
        file << "- type: INFO\n";
        add_timestamp(event.time_point);
        file << "  info: \"" << event.info << "\"\n";
    }

    void operator()(const eviso15118::session_2_sap::logging::ExiMessageEvent& event) {
        file << "- type: EXI\n";
        add_timestamp(event.time_point);
...
private:
    ...
    void add_timestamp(const eviso15118::session_2_sap::logging::TimePoint& timestamp) {
    ...
    eviso15118::session_2_sap::logging::TimePoint last_timestamp;
...
SessionLogger::SessionLogger(std::filesystem::path output_dir_) : output_dir(std::filesystem::absolute(output_dir_)) {
    ...
    eviso15118::session_2_sap::logging::set_session_log_callback([this](std::uintptr_t id, const LogEvent_2_sap& event) {
    ...
    eviso15118::session_2::logging::set_session_log_callback([this](std::uintptr_t id, const LogEvent_2& event) {
        auto log_it = logs.find(id);
        if (log_it == logs.end()) {
            const auto log_file_name = output_dir / get_filename_for_current_time();
            const auto emplaced = logs.emplace(id, std::make_unique<SessionLog>(log_file_name.string()));
